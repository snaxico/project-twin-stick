...
const PYLON_LAYOUTS := [
	[Vector2(820, 560), Vector2(2780, 560), Vector2(820, 1540), Vector2(2780, 1540)],
	[Vector2(620, 500), Vector2(1500, 1550), Vector2(2600, 450), Vector2(3050, 1350)],
	[Vector2(500, 1100), Vector2(1350, 450), Vector2(2300, 1550), Vector2(3100, 700)],
]
...
func setup(arena: Rect2, players: Array, coop: Node) -> void:
	...
	for pylon_variant in PYLON_LAYOUTS[_variant_index]:
		_pylons.append(_arena.position + pylon_variant as Vector2)
...
func _apply_hits() -> void:
	...
	for actor in _players:
		...
		for pair_variant in schedule:
			...
			if _point_segment_distance(pos, _pylons[int(pair[0])], _pylons[int(pair[1])]) <= ARC_W:
				hit[actor] = true
				break
	for actor in hit.keys():
		_damage_player(actor, 9)
...
func _draw() -> void:
	...
	for pair_variant in schedule:
		var pair: Array = pair_variant as Array
		draw_line(_pylons[int(pair[0])], _pylons[int(pair[1])], color, ARC_W if live else 10.0)
