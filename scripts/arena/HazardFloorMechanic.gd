...
func _physics_process(delta: float) -> void:
	...
	if _cycle_elapsed >= CYCLE_TIME:
		...
		_set_index = (_set_index + 1) % SAFE_ZONE_SETS.size()
		_spent_mines.clear()

	if _variant == "mine_grid":
		_update_mines()
	else:
...
func _update_floor_effect(delta: float) -> void:
	...
	for player in _players:
		...
		if should_damage and off_zone:
			_damage_player(player, 5 if _variant == "fire_grid" else 3)
...
func _is_in_safe_zone(position: Vector2, set_index: int) -> bool:
	...
	for center_variant in SAFE_ZONE_SETS[set_index]:
...
func _build_mine_sets() -> void:
	_mine_sets.clear()
	for set_index in range(SAFE_ZONE_SETS.size()):
...
func _draw() -> void:
	...
	if _cycle_elapsed >= HOLD_TIME:
		_draw_safe_zones((_set_index + 1) % SAFE_ZONE_SETS.size(), Color(1.0, 0.72, 0.2, 0.78), true)
	if _variant == "mine_grid" and not _mine_sets.is_empty():
		_draw_mines(_set_index, false)
		if _cycle_elapsed >= HOLD_TIME:
			_draw_mines((_set_index + 1) % SAFE_ZONE_SETS.size(), true)
...
func _skin_colors() -> Dictionary:
	match _variant:
		"frost_grid":
			return {"hazard": Color(0.12, 0.35, 0.58, 0.3), "safe": Color(0.62, 0.9, 1.0, 0.85)}
		"mine_grid":
			return {"hazard": Color(0.38, 0.29, 0.08, 0.24), "safe": Color(1.0, 0.9, 0.46, 0.85)}
		_:
			return {"hazard": Color(0.62, 0.12, 0.05, 0.3), "safe": Color(1.0, 0.72, 0.32, 0.85)}
