...
- **Work in `D:\GameDev\Project_Twin_stick` on `v4/class-system`** (the mainline / GitHub default). Treat
  this repo as the current game.
- **No new git worktrees** unless explicitly requested. Stale agent worktrees under `.claude/worktrees/`
  should be removed, not added to.
- Older `v2/*` and `v3/*` branches are historical baselines/reference material. Do not merge them into V4
  wholesale; cherry-pick or manually port only reviewed changes that still fit the class-system direction.
...
4. Validate, then commit on `v4/class-system`.
